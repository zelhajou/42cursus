...
def nombreDeNoeudsParNiveau (A,n):
    if A!=None:
        if n==0:return 1
            ...
            return nombreDeNoeudsParNiveau (A[1][0],n-1)+\
                   nombreDeNoeudsParNiveau (A[1][1],n-1)
    else:return 0
...
def chercher(x,A):
    if A!=None:
        ...
        else:return chercher(x,A[1][0]) or chercher(x,A[1][1])
    else:return False
        
def listeDenoeudsParNiveau(A,n):
    if A!=None:
        if n==0:return [A[0]]
        else:return listeDenoeudsParNiveau(A[1][0],n-1)+listeDenoeudsParNiveau(A[1][1],n-1)
    else:return [None]
...
def listInordre(A):
    if A==None:return []
    else:return listInordre(A[1][0])+[A[0]]+listInordre(A[1][1])
...
def transformerListe(L,A,i=0):
    ...
    if 2*i+1<len(L):
        inserer(A,L[2*i+1],"g")
        transformerListe(L,A[1][0],2*i+1)
    if 2*i+2<len(L):
        inserer(A,L[2*i+2],"d")
        transformerListe(L,A[1][1],2*i+2)
...
inserer(A[1][0][1][0],8,"g")
inserer(A[1][0][1][0],9,"d")

inserer(A[1][0][1][0][1][0],0,"g")
